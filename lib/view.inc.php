...
<script type="text/javascript" src="js/js.cookie.js"></script>
 ...
 <body><div class="row" style="width:'.($width + 82).'px"><div class="channel empty"><h2><a href="/'.($_GET['page'] == 1 ? '' : '?page='.($_GET['page']-1) ).'">&Lang;</a></h2></div><div class="header" style="width:'.$hour_width.'px"><h2>'.date('H:00', $start_timestamp).'</h2></div><div class="header" style="width:'.$hour_width.'px"><h2>'.date('H:00', $start_timestamp + 3600).'</h2></div><div class="header" style="width:'.$hour_width.'px"><h2>'.date('H:00', $start_timestamp + 7200).'</h2></div><div class="header" style="width:'.($hour_width-81).'px"><h2>'.date('H:00', $start_timestamp + 10800).'</h2></div><div class="channel empty right"><h2><a href="/'.($_GET['page'] == -1 ? '' : '?page='.($_GET['page']+1) ).'">&Rang;</a></h2></div></div><div class="row" style="width:'.($width+82).'px">';
...
while($row = $result->fetch_object()){
  if($previous_channel && $row->channel != $previous_channel){
    echo '</div><div class="row" style="width:'.($width + 82).'px">';
  }
  if(!$previous_channel || $row->channel != $previous_channel){
  ...
  echo '<div class="programme '.implode(' ', $categories).'" style="width:'.calculate_width($row->start, $row->stop, $now, $now4, $width).'px" title="'.get_tooltip_text($row).'"><p>'.get_title_text($row).'</p></div>';
}
echo '
  </div>
 </body>
